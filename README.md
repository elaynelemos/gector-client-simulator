...
curl -X POST -H "Content-Type: application/json" -d '{"sentence": "In conclusion , although the unsafe online environment , spend too much time on internet may be the potential negative factors of using online social networks .\n"}' http://52.186.65.143/correct

python3 sentences_correction.py http://52.186.65.143 1 outputs/standard-f4s-v2

QUERY_MEMORY='(1 - ((avg_over_time(node_memory_MemFree_bytes{job="standard-f4s-v2"}[30s]) + avg_over_time(node_memory_Cached_bytes{ job="standard-f4s-v2"}[30s]) + avg_over_time( node_memory_Buffers_bytes{job="standard-f4s-v2"}[30s])) / avg_over_time(node_memory_MemTotal_bytes{job="standard-f4s-v2"} [30s])))'

QUERY_CPU='(avg without (mode,cpu) (1 - rate(node_cpu_seconds_total{mode="idle", job="standard-f4s-v2"}[30s])))'
